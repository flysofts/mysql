    ...
    if(req.method === 'POST'){
      ...
      try{
        ...
        if(!parentid || !userid || !username || !content){
          ...
          const [results] =await db.query<RowDataPacket[]>('insert into parkjihawn.comment (parentid, userid, username, content) values (?,?,?,?)',[parentid, userid, username, content])
  ...
  export const GET = async (
    ...
    if(req.method === 'GET'){
      try{  
        ...
        ('select * from parkjihawn.comment where parentid = ?',[parentid]);
